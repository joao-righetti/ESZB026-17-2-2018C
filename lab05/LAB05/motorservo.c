...
int main() {                            // este programa deve ser rodado com 'sudo'
   ...
   // Ajustando a frequencia do PWM em 261.6 Hz com 128 passos de duty cycle
   // frequencia PWM = 19,2 MHz / (divisor * range)
   // 50= 19200000 / (divisor * 128) => divisor = 573.4
   pwmSetMode(PWM_MODE_MS);             // usando frequencia fixa
   pwmSetRange(620);                    // passos do duty cycle (max=4096)
   pwmSetClock(619);                     // fornece uma frequencia de 10kHz (max=4096)
   for(dc = 0; dc <= 600; dc=dc+100){
   	printf("Iniciando...%d\n",dc);      
